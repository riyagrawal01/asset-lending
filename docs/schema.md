...
* `passwordHash` is stored as a hash rather than plaintext. Password hashing will be handled by the authentication service in M03.
...
The transition definitions are present in the model. Enforcement of transitions belongs to the loan service layer and will be implemented in M05.
...
* Application-level protection against updating or deleting events will be implemented with the history functionality.
...
| Loan          | `{ item: 1, status: 1 }`           | Find open loans when checking item availability           |
...
The standalone `{ item: 1 }` ItemCustodian index is also not needed because `item` is the first field in the existing compound index.
